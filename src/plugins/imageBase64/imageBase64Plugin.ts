...
import { ImageBase64Content } from './imageBase64Content';
...
export class ImageBase64Plugin extends TemplatePlugin {

    public readonly contentType = 'imageBase64';

    public async simpleTagReplacements(tag: Tag, data: ScopeData, context: TemplateContext): Promise<void> {

        const wordTextNode = this.utilities.docxParser.containingTextNode(tag.xmlTextNode);

        const content = data.getScopeData<ImageBase64Content>();
        ...
        const mediaFilePath = await context.docx.mediaFiles.add(Buffer.from(content.source, 'base64'), content.format);
    ...
    private createMarkup(imageId: number, relId: string, content: ImageBase64Content): XmlNode {
    ...
    private docProperties(imageId: number, name: string, content: ImageBase64Content): string {
        if (content.altText) {
    ...
    private pictureMarkup(imageId: number, relId: string, name: string, content: ImageBase64Content) {
